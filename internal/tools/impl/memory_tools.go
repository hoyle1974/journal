...
func registerKnowledgeTools() {
	tools.Register(&tools.Tool{
		...
		Description: "Manual override tool for knowledge corrections. Use ONLY when the user gives an explicit command to remember/store/save/correct/update a specific fact. Do NOT use this for general conversation or routine declarative statements; those are handled automatically by ingest-time Refinery extraction. Node types: 'person', 'project', 'goal', 'preference', 'event', 'milestone', 'place', 'asset', 'tool', 'generic', 'user_identity'. For node_type 'project' or 'goal', metadata.status must be exactly one of: active, blocked, done, planning, pending, completed (e.g. {\"status\": \"active\"}). Use node_type 'user_identity' for self-referential identity facts. Relational facts can be stored as SPO triples by supplying optional predicate (e.g. 'works_at') and object_value (e.g. 'Google').",
		Category:    "knowledge",
