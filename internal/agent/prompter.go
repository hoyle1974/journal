...
func BuildSystemPrompt(ctx context.Context, env infra.ToolEnv, ragContext string) (string, error) {
	...
	// 5. Recent Reflections (Dreamer summary nodes)
	recentReflectionsBlock := ""
	if env != nil {
		if summaries, err := GetRecentSummaries(ctx, env, 3); err == nil && len(summaries) > 0 {
			recentReflectionsBlock = formatReflectionsSection(summaries)
			recentReflectionsBlock = utils.WrapAsUserData(recentReflectionsBlock)
		}
	}
	...
	infra.LoggerFrom(ctx).Info("LLM_CONTEXT_SENT | injected context sections (Conversation, Alerts, Gaps, Tasks, Project, Reflections)",
	...
	promptData := prompts.SystemPromptData{
		...
		RecentReflectionsBlock: recentReflectionsBlock,
	}
...
// formatReflectionsSection builds the RECENT REFLECTIONS block from Dreamer summary nodes.
func formatReflectionsSection(summaries []string) string {
	if len(summaries) == 0 {
		return ""
	}
	var lines []string
	...
	return fmt.Sprintf("\n---\n## 🧠 RECENT REFLECTIONS\n# Summaries of recent activity generated by the background dream cycle.\n\n%s", strings.Join(lines, "\n"))
}
