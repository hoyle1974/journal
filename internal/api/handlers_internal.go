...
func handleProcessEntry(s *Server, w http.ResponseWriter, r *http.Request) {
	...
	defer cancel()
	breakdown, err := s.Backend.ProcessEntry(ctx, data.UUID, data.Content, data.Timestamp, data.Source)
...
func handleDraftTools(s *Server, w http.ResponseWriter, r *http.Request) {
	...
	drafts, err := s.Backend.GetDraftTools(ctx)
	if err != nil {
		LogHandlerResponse(ctx, r.Method, path, http.StatusInternalServerError, "error", err.Error())
		WriteJSON(w, http.StatusInternalServerError, map[string]string{"error": err.Error()})
		return
	}
	LogHandlerResponse(ctx, r.Method, path, http.StatusOK, "count", len(drafts))
...
func handleDraftToolApply(s *Server, w http.ResponseWriter, r *http.Request) {
	...
	LogHandlerRequest(ctx, r.Method, path, "uuid", data.UUID)
	if err := s.Backend.MarkToolDraftApplied(ctx, data.UUID); err != nil {
		LogHandlerResponse(ctx, r.Method, path, http.StatusInternalServerError, "error", err.Error())
		WriteJSON(w, http.StatusInternalServerError, map[string]string{"error": err.Error()})
		return
	}
