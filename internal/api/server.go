...
// Implemented by *infra.App (via jot) so that handlers can enqueue Cloud Tasks (e.g. process-telegram-query).
...
// NewServer builds a Server with validator and chi router. Domain services must be non-nil for handlers that use them.
func NewServer(app AppLike, cfg *config.Config, logger *slog.Logger, journal JournalService, memory MemoryService, agent AgentService, telegram TelegramService, system SystemService) *Server {
