// show_logged scans all journal entries and prints those whose content contains "Logged".
...
func runShowLogged(ctx context.Context, app *infra.App, args []string) {
	const batchLimit = 500
	fs := flag.NewFlagSet("show-logged", flag.ExitOnError)
	previewLen := fs.Int("preview", 120, "Max characters of content to show per entry")
	remove := fs.Bool("remove", false, "Delete entries containing \"Logged\"")
	...
	for {
		...
		if !strings.Contains(content, "Logged") {
			continue
		}
	...
	if *remove {
		...
		fmt.Printf("Deleted %d entries containing \"Logged\".\n", count)
	} else {
		fmt.Printf("Found %d entries containing \"Logged\".\n", count)
	}
}
