...
2. **Loop:** LLM either answers or issues tool calls. Optional `<thought>...</thought>` blocks are stripped from model text before tool/answer parsing; extracted text is accumulated as `reasoning_trace` on the query result (truncated per iteration for trace size). The root `query.run` span is updated with `foh_iteration` / `foh_last_thought_len` each iteration. Tools run in parallel (worker pool); results are sent back to the LLM. `ChatSession.TrimHistory` is a no-op with the current `genai.Chat` SDK — context growth is managed by concise-thought guidance in the system prompt, not by server-side history trimming.
3. **Unified audit:** The system prompt requires the model to perform reflection, gap detection, and synthesis in its reasoning before giving the final answer. If the model outputs `MISSING_INFO: <list>`, that is parsed and the query is saved as a knowledge gap. Explicit non-empty **Identified gaps:** lines inside `<thought>` also set the gap flag. Graph neighbourhood expansion is **not** auto-injected after `semantic_search`; the model calls `graph_expand` when needed (compact tools include `graph_expand`).
