...
func TestValidateMetadata(t *testing.T) {
	...
	tests := []struct {
		...
		// Typed error prefix assertions — validate that VALIDATION_FAILED appears in the rendered error.
		{name: "preference invalid category returns MemoryError", nodeType: "preference", meta: map[string]any{"category": "music"}, wantErr: true, errSubstr: "VALIDATION_FAILED"},
		{name: "project invalid status returns MemoryError", nodeType: "project", meta: map[string]any{"status": "wip"}, wantErr: true, errSubstr: "VALIDATION_FAILED"},
	}
